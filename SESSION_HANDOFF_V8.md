# HorizonIQ — Session Handoff V8

**Last updated:** 2026-06-28  
**Version:** MVP V1.1 (Change-First) + Live Pipeline + Sprints 1–3C (local)  
**Status:** Active Development — **Deployed to Vercel (beta)** + local dev with uncommitted Sprint 3C polish  
**Purpose:** Zero-context handoff for the next chat session or engineer.  
**Supersedes:** `SESSION_HANDOFF_V7.md`, `SESSION_HANDOFF_V6.md`, and earlier handoffs
...
| **Local uncommitted** | Sprint 3C Living Intelligence Core (redesigned), React 19 + R3F v9.5, WebGL on all pages, guided tour fix (portal), animation readability overhaul, onboarding UX fixes, `start-dev.bat`, `next.config.mjs`, `.npmrc`, doc updates |
...
**Sprint 3C elevation:** HorizonIQ is becoming a **Personal Intelligence Operating System** — not a dashboard. The signature visual is the **Living Intelligence Core**: an abstract, data-reactive intelligence field (wireframe mesh + sparse nodes — not a globe, brain, or neural-network cliché).
...
| Guided tour completion | Did tour help comprehension? | **Instrumented locally** — `guided_tour_completed` |
...
### Living Intelligence Core Contract (Sprint 3C — Redesigned)

| Visual behavior | Data source |
|---|---|
| Wireframe icosahedron rotation | Ambient motion (mesh) |
| Sparse particle nodes on sphere | Signal momentum + change ratio |
| Brightness / opacity | Average confidence tier |
...
| Mode | Behavior |
|---|---|
| `css` | Procedural CSS fallback — reliable, no WebGL |
| `webgl` | Living Intelligence Core (R3F + GLSL mesh + points) with error boundary |
| `auto` | `webgl` on welcome; `css` elsewhere |

**Current page assignments (local):**

| Page | Mode | Variant | Notes |
|---|---|---|---|
| `/` landing | `webgl` | `hero` | Wireframe + sparse particles; readability overlay |
| `/dashboard` | `webgl` | `ambient` | Data-reactive; lower intensity |
| `/signals/[id]` | `webgl` | `ambient` | Single-signal reactive |
| `/onboarding/welcome` | `webgl` | `welcome` | Phased reveal; CSS fallback on error / reduced motion |

**Animation design (post-redesign):**

- ~220 sparse particles on dashboard (not ~9,000 dense additive cloud)
- Wireframe icosahedron mesh (`IntelligenceFieldMesh`) for structure
- `IntelligenceFieldOverlay` — radial/linear scrims for text readability
- `NormalBlending` on particles (not additive)
- CSS fallback matches aesthetic when WebGL unavailable
...
/ (landing)
  ...
  → /onboarding/region           [region; Continue → interests OR Quick start → tour]
  → /onboarding/interests        [optional — custom path; nothing pre-selected]
  → /onboarding/tour             [guided tour vs explore]
  → /dashboard                   [story hero + context bar + depth + optional guided tour]
  → /signals/[id]                [full IntelligenceCard]
```

**Quick path:** welcome → name → greeting → role → region → **Quick start** → tour → dashboard.

**Custom path:** welcome → name → greeting → role → region → **Continue** → select focus areas → tour → dashboard.
...
- [x] Intelligence field background — WebGL with CSS fallback (Sprint 3C local)
...
### Guided Tour (local — rebuilt and verified)

- [x] **4-step tour** — briefing hero → watchlist → next briefing → recommended action
- [x] **SVG mask spotlight** with highlight ring
- [x] **`createPortal` to `document.body`** — fixes `position: fixed` trapped by page-transition transform
- [x] **Decoupled scroll from measure** — scroll once per step; re-measure on scroll/resize without re-scrolling
- [x] **Viewport-safe card placement** — below spotlight, else above, else centered
- [x] **Small stable `data-tour` targets** — headers and `p` tags, not full sections
- [x] **Keyboard support** — Esc skips, Enter/→ advances
- [x] **`guided_tour_completed`** analytics event (skipped + stepsCompleted)
- [x] Tour deferred until dashboard layout ready (900ms delay)
- [x] Visit snapshot deferred until tour complete

### Onboarding UX Fixes (local)

- [x] **`FadeIn` / `Stagger` `immediate` prop** — cards animate on mount, not only on scroll-into-view
- [x] **No pre-selected interests** — user must choose focus areas explicitly
- [x] **Region page flow clarified** — "Continue" → interests (clears selections); "Quick start" → tour with role defaults
- [x] **Interests page** waits for hydration + role before rendering
...
- [x] Step progress step 3 → **Focus Areas**
...
- [x] **Living Intelligence Core** — R3F + GLSL wireframe mesh + sparse particle field
- [x] **`IntelligenceFieldLayer`** — lazy-loaded with error boundary
- [x] **`IntelligenceFieldMesh`** — wireframe icosahedron
- [x] **`IntelligenceFieldOverlay`** — readability scrims + subtle rings/bokeh
- [x] **`components/intelligence-field/types.ts`** — shared types/constants (breaks circular deps)
...
- [x] **WebGL on landing, dashboard, signal detail** — with graceful CSS fallback
- [x] **React 19** + **`@react-three/fiber@9.5.0`** — required for Next.js 15 compatibility
- [x] **`next.config.mjs`** — transpilePackages + client React dedupe + dev snapshot config
- [x] **`npm run dev:clean`** — clears stale `.next` cache
- [x] **`start-dev.bat`** — kills port 3000, clears cache, waits for HTTP 200, opens browser
...
- [ ] **Commit all local Sprint 3C + fixes** to `main`
...
- [ ] Smoke-test guided tour on production (portal fix + scroll behavior)
...
- [ ] Onboarding continuity — ambient field on role/region steps

### Post-Sprint Polish

- [ ] **Display name on dashboard** — baseline banner / hero salutation from `identityService`
- [ ] **PostHog funnels** — Week 2 return, `path: quick` vs `custom`, change-hero → signal-detail, guided tour completion
...
### Guided Tour Flow (Fixed)

```
identityService.shouldShowGuidedTour()
        ↓
...
guided-tour-overlay.tsx          → createPortal(..., document.body)
        ...
        └─ Card: viewport-safe placement (below / above / center)
        ↓
identityService.completeGuidedTour() → guidedTourCompletedAt persisted
```

**Critical fix:** `app/template.tsx` applies `animate-page-in` (CSS transform). Any `position: fixed` overlay inside that tree scrolls with the page. The tour **must** portal to `document.body`.
...
- Dev `snapshot.managedPaths` for `node_modules` — reduces stale module cache issues
...
├── start-dev.bat                        # Robust local dev launcher
│
├── app/
│   ├── page.tsx                         # Landing — IntelligenceFieldLayer mode=webgl hero
│   ├── layout.tsx                       # React 19, Outfit+Inter, dark default, Providers
│   ├── template.tsx                     # CSS page-in animation (3B) — creates transform context
│   ├── globals.css                      # Design tokens, premium palette (3B)
│   ├── dashboard/page.tsx               # Story hero + field + guided tour + progressive depth
│   ├── onboarding/
│   │   ├── welcome/page.tsx             # WelcomeScreen — Living Core reveal
│   │   ├── name/page.tsx
│   │   ├── greeting/page.tsx
│   │   ├── role/page.tsx                # Stagger immediate
│   │   ├── region/page.tsx              # Continue vs Quick start
│   │   ├── interests/page.tsx           # No pre-selection; Stagger immediate
│   │   └── tour/page.tsx
...
│   │   ├── types.ts                     # Shared types + FIELD_INTENSITY
...
│   │   ├── intelligence-field-overlay.tsx   # Readability scrims
│   │   ├── intelligence-field-mesh.tsx      # Wireframe icosahedron
│   │   ├── intelligence-field-points.tsx    # Sparse GLSL particles
...
│   │   ├── guided-tour-overlay.tsx      # Portal + SVG mask spotlight (fixed)
│   │   ├── onboarding-shell.tsx         # FadeIn immediate
│   │   ├── step-progress.tsx            # Focus Areas label (3A polish)
│   │   └── observe-understand-act-steps.tsx
│   ├── dashboard/
│   │   ├── what-changed-hero.tsx        # RETENTION CORE — story acts + data-tour targets
│   │   ├── story-section.tsx
...
│   │   ├── signals-we-are-tracking.tsx  # Watchlist + next-briefing tour targets
...
│   ├── motion/                          # fade-in.tsx (immediate prop), animated-counter
│   ├── theme/, ui/
│
├── hooks/
...
├── lib/
│   ├── identity/
│   ├── analytics/                       # events.ts includes guided_tour_completed
│   ├── data/
│   ├── pipeline/
...
├── SESSION_HANDOFF_V8.md                # This file
├── SESSION_HANDOFF_V7.md                # Superseded
└── .cursorrules
```
...
6. **Never prioritize visual effects over usability** — text must remain readable
...
| `IntelligenceFieldOverlay` | Readability scrims over WebGL field |
| Living Intelligence Core | Wireframe mesh + sparse nodes (WebGL with CSS fallback) |
...
### Sprint 3C — Animation (Redesigned)

| Decision | Rationale |
|---|---|
| Wireframe icosahedron + sparse particles | Structured, professional — not chaotic particle cloud |
| ~220 particles on dashboard | Readable text over animation |
| `IntelligenceFieldOverlay` scrims | Radial/linear gradients protect text contrast |
| `NormalBlending` on particles | Avoids additive wash that obscures content |
| Lower ambient intensity (0.32) | Dashboard is for reading, not spectacle |
| CSS fallback matches aesthetic | Graceful degradation when WebGL fails |
...
| Surface | Decisions |
|---|---|
| Landing | Tagline lockup + Beta badge + hero; WebGL field + overlay; content `z-10` stack |
| Welcome | Phased Living Core reveal; WebGL with fallback |
| Name / Greeting | `FirstTimeShell` — minimal |
| Profile onboarding | Role · Region · Focus Areas; immediate Stagger animations |
| Tour choice | Guided vs explore |
| **Dashboard (all visits)** | Baseline (first) → **Story hero** → **Context bar** → depth |
| **Dashboard (first)** | + watchlist + optional guided tour |
...
|---|---|---|
| `what-changed` | Briefing hero `<header>` | Step 1 |
| `watchlist` | Watchlist header `<div>` | Step 2 |
| `next-briefing` | "Your next briefing will reveal:" `<p>` | Step 3 |
| `recommended-actions` | Primary action title `<p>` in hero | Step 4 |
| `briefing-signals` | Change list in hero | Not in tour (reference only) |
| `dashboard` | Root dashboard container | Not in tour (reference only) |
| `opportunities` | Inside disclosure (return visits) | Not in tour (reference only) |
...
| Guided tour | Optional; portal overlay; deferred snapshot until complete |
...
|---|---|---|
| Sprint 3C + fixes uncommitted | **High** | Production lacks Living Core redesign, guided tour fix, React 19 |
...
| Guided tour step 4 scrolls up to hero action | Low | By design — action is in hero, not watchlist |

### Technical

| Issue | Severity | Notes |
|---|---|---|
| **`position: fixed` trapped by page transform** | **Fixed locally** | Tour portals to `document.body` |
| **Scroll feedback loop in tour** | **Fixed locally** | Decoupled scroll from measure |
| **R3F v8 incompatible with Next.js 15** | **Fixed locally** | React 19 + `@react-three/fiber@9.5.0` |
| **`ReactCurrentOwner` runtime error** | **Fixed locally** | React upgrade + webpack alias |
| **`__webpack_modules__` TypeError** | **Mitigated locally** | `types.ts` breaks circular deps; `dev:clean` clears cache |
| Stale `.next` cache crashes | Medium | `ENOENT _document.js` — run `npm run dev:clean` or `start-dev.bat` |
...
### 1. Commit and deploy all local work to `main`

**Why:** Production is on `44c1c84` (Sprint 3B only). Local has Sprint 3C redesign, React 19, guided tour fix, onboarding UX fixes, and `start-dev.bat`. Nothing ships until committed.  
**Do:** Stage all changes, commit with clear message, push `main`, verify Vercel build succeeds, smoke-test guided tour + WebGL on production.

### 2. Verify guided tour on production across viewport sizes

**Why:** Tour was broken for weeks due to transform-trapped overlay. Fix is verified locally but must hold on Vercel with real users/devices.  
**Do:** Complete onboarding → Start Guided Tour → advance all 4 steps on desktop and mobile widths. Confirm spotlight, card, and buttons work after scroll.

### 3. Use display name on dashboard

**Why:** Closes the personalization loop from name capture during onboarding. Reinforces "your" intelligence platform.  
**Do:** Add time-of-day salutation or first-name reference in baseline banner or hero subline via `identityService.getDisplayName()`.

### 4. PostHog funnels and retention insights

**Why:** MVP validates habit — need Week 2 return, onboarding path comparison, change-hero → signal-detail, and guided tour completion rates.  
**Do:** Create PostHog insights/dashboards for primary metrics. Confirm `guided_tour_completed` fires on production.

### 5. Stabilize WebGL performance (shared canvas + GPU tier)

**Why:** WebGL now runs on landing, dashboard, and signal detail. Each navigation remounts a canvas — performance risk on lower-end devices.  
**Do:** Implement single shared WebGL context across routes; add GPU tier detection for particle budget; optional dev FPS overlay.
...
**Important:** If you see `ReactCurrentOwner`, `ENOENT _document.js`, `__webpack_modules__`, or blank pages:

1. Stop **all** running `next dev` processes  
2. `Remove-Item -Recurse -Force .next`  
3. `npm install`  
4. `npm run dev:clean` or `.\start-dev.bat`  
...
3. Role → Region → **Continue** (select focus areas) OR **Quick start**  
4. Tour choice → **Start Guided Tour**  
5. Dashboard → complete all 4 tour steps (verify spotlight tracks after scroll)  
6. Return to `/dashboard` → expect return-visit story + disclosure sections  

### Test Guided Tour Specifically

```javascript
// Browser console — arm guided tour without full onboarding
const id = JSON.parse(localStorage.getItem('horizoniq.identity.v1') || '{}');
id.tourChoice = 'guided';
id.guidedTourCompletedAt = null;
localStorage.setItem('horizoniq.identity.v1', JSON.stringify(id));
location.href = '/dashboard';
```

Verify:

- Overlay renders above all content (not scrolling with page)
- Spotlight ring aligns with target element
- Card is visible and buttons are clickable
- `guidedTourCompletedAt` is set after Finish
...
**Local uncommitted:** Sprint 3C Living Intelligence Core (redesigned), React 19, R3F v9.5, WebGL on all pages, guided tour portal fix, animation readability overhaul, onboarding UX fixes, `start-dev.bat`, `next.config.mjs`, `.npmrc`, `guided_tour_completed` event, doc updates.
...
| `SESSION_HANDOFF_V7.md` | Superseded handoff |
...
| Wireframe mesh + sparse nodes (readable over text) | Locked (3C redesign) |
| Not a globe, brain, or neural-network cliché | Locked (3C) |
| WebGL with CSS fallback when unsafe | Locked (3C) |
| Guided tour portals to document.body | Locked (3C fix) |
...
**Living Core (3C):** Data-reactive wireframe field; WebGL on all main pages with CSS fallback.
...
Quick-start: skip focus areas step with role defaults. Interests never pre-selected on custom path.
...
| Sprint 1 onboarding + tour | On `main` (tour fix local) |
...
| Guided tour portal fix | **Local uncommitted** |
| Animation readability redesign | **Local uncommitted** |
| Onboarding UX fixes | **Local uncommitted** |
...
- Visual effects must not block page load or obscure text (Sprint 3C)  

---

*End of Session Handoff V8*
